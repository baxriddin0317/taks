import { useContext, useState } from 'react'
import { Link } from 'react-router-dom';
import { AuthContext } from '../../context/AuthContext';
import LoginModal from '../modals/LoginModal';
import SignUpModal from '../modals/SignUpModal';

const Navigation = () => {
  const [ openModal, setOpenModal ] = useState(false);
  const [ openLogin, setOpenLogin ] = useState(false);

  // get user data and logout function on Authcontext
  const {user, logout} = useContext(AuthContext)

  return (
    <header className='container mx-auto py-10 mb-14 xl:px-3.5'>
      <div className='flex items-center justify-between'>
        {/* site logo */}
        <Link to={'/'}>
          <h1 className='font-bold italic text-brand-mainColor text-3xl cursor-pointer underline'>
            Tasks
          </h1>
        </Link>

        {/* site menu */}
        <ul className='flex items-center gap-x-15'>
          {!user ? (
            <li className='flex gap-5'>
              <button
                onClick={()=>setOpenLogin(true)} 
                className='bg-transparent text-brand-mainColor text-lg duration-500 capitalize border border-transparent hover:border-brand-mainColor hover:shadow-md hover:shadow-brand-mainColor px-5 hover:rounded-md'
              >
                login up
              </button>
              <button 
                onClick={()=>setOpenModal(true)} 
                className='btn-hover bg-brand-mainColor text-white py-3.5 px-7 rounded-md text-lg capitalize'
              >
                sign up
              </button>
            </li>
          ) : (
            <li className='flex gap-5'>
              <button
                onClick={logout} 
                className='bg-transparent text-brand-mainColor text-lg duration-500 capitalize border border-transparent hover:border-brand-mainColor hover:shadow-md hover:shadow-brand-mainColor px-5 hover:rounded-md'
              >
                logout
              </button>
              <div className='flex gap-1 items-center'>
                <p>{user.username}</p>
                <div className='relative w-10 h-10 rounded-full overflow-hidden'>
                  <img className='object-cover' alt='alik' src={'data:image/jpeg;base64,'} />
                </div>
              </div>
            </li>
          )
        }
        </ul>
      </div>

      {openModal && <SignUpModal setOpenModal={setOpenModal} setOpenLogin={setOpenLogin} />}
      {openLogin && <LoginModal setOpenLogin={setOpenLogin} />}
    </header>
  )
}

export default Navigation